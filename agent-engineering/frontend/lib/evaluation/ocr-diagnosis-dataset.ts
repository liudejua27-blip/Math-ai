export type OCRNoiseTag =
  | "spaced_latex"
  | "full_width_symbol"
  | "step_index_noise"
  | "delta_inequality_noise"
  | "geometry_reference_noise"
...
export const OCR_DIAGNOSIS_CASES: OCRDiagnosisCase[] = [
  {
    ...
    confirmedStudentSteps:
      "S1: f'(x)=1/x-a。\nS2: 代入 x=1 得 f'(1)=1-a。",
    noiseTags: ["spaced_latex", "full_width_symbol", "step_index_noise"],
    expected: {
      firstWrongStep: "S1",
      atomIds: ["A02"],
    },
    notes:
      "真实草稿纸常把 ln 拆成 l n，也会混入全角符号和手写步骤编号；确认后应稳定定位求导公式错误。",
  },
  {
    id: "OCR-Q01",
    topic: "quadratic",
    ...
    confirmedProblemText: "已知二次函数 y=x^2-2ax+1 对任意实数 x 恒大于 0，求 a 的范围。",
    confirmedStudentSteps:
    ...
    notes:
      "参数恒成立题的草稿 OCR 常把 Δ、全角不等号和步骤编号识别不稳定；需要检查条件提取、开口方向和参数范围。",
  },
  {
    id: "OCR-G01",
    ...
    confirmedStudentSteps:
      "S1: 连接 A1C。\nS2: 直接把 A1C 和 AB 的夹角当成线面角，所以答案是 45°。",
    noiseTags: ["geometry_reference_noise", "low_confidence_formula"],
    expected: {
      firstWrongStep: "S2",
      atomIds: ["A32"],
    },
    notes:
      "几何草稿识别要保留关键线段、面名和角的对象，避免把普通夹角误当线面角。",
  },
];
